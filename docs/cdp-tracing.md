...
| --- | --- | --- |
| Header name aligns FE ↔ BE | ✅ Both use `x-cdp-request-id` | FE: [`src/config/config.js`](../../epr-register-case-management-frontend-poc/src/config/config.js) `tracing.header` default; BE: [`appsettings.json`](../Backend.Api/appsettings.json) `TraceHeader` |
| Inbound header is propagated on outbound HttpClients | ✅ Wired via `AddHeaderPropagation` | [`Program.cs`](../Backend.Api/Program.cs) `ConfigureHeaderPropagation` |
| Logs enriched with trace id | ✅ Serilog `Enrich.WithCorrelationId(traceIdHeader)` | [`Utils/Logging/CdpLogging.cs`](../Backend.Api/Utils/Logging/CdpLogging.cs) |
...
1. Register via `services.AddHttpClientWithTracing<IFoo, FooClient>();` (see
   `Backend.Api/Utils/Http/HttpClientExtensions.cs`).
