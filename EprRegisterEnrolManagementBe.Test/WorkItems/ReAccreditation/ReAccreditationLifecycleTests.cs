...
public class ReAccreditationLifecycleTests
{
    ...
    public async Task Walk_from_submitted_to_approved_completing_every_task_records_full_audit_trail()
    {
        ...
        // 2 + 1 + 3 + 1 = 7 task completions + 3 engine transitions (duly-make,
        // payment-received, submit-for-decision) + 3 approval audit entries
        // (action-applied, sla-clock-stopped, accreditation-issued) = 13 total.
        ...
        Assert.Contains(workItem.AuditLog, e => e.Action == "action-applied"
            && e.Details.GetValueOrDefault("actionId") == "submit-for-decision");
        Assert.Contains(workItem.AuditLog, e => e.Action == "action-applied"
